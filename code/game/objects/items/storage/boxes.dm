...
/obj/item/storage/box/attack_self(mob/user)
	...
	if(!foldable)
		return
	if(contents.len)
		to_chat(user, span_warning("I can't fold this box with items still inside!"))
		return
	if(!ispath(foldable))
		return
	...
	if(istype(W, /obj/item/pen))
		...
		switch(choice)
			...
			if("SmileyFace")
				...
				return FALSE
		to_chat(user, span_notice("I make some modifications to [src] using your pen."))
		...
		if(!contents.len)
			if(inhand_icon_state == "paperbag_None")
				user.show_message(span_notice("I cut eyeholes into [src]."), MSG_VISUAL)
				new /obj/item/clothing/head/papersack(user.loc)
				qdel(src)
				return FALSE
			else if(inhand_icon_state == "paperbag_SmileyFace")
				user.show_message(span_notice("I cut eyeholes into [src] and modify the design."), MSG_VISUAL)
...
/obj/item/storage/box/papersack/proc/check_menu(mob/user, obj/item/pen/P)
	...
	if(user.incapacitated())
		return FALSE
	if(contents.len)
		to_chat(user, span_warning("I can't modify [src] with items still inside!"))
		return FALSE
	if(!P || !user.is_holding(P))
		to_chat(user, span_warning("I need a pen to modify [src]!"))
		return FALSE
	return TRUE
...
/obj/item/secbat/attack_self(mob/user)
	...
	to_chat(user, span_notice("I open the box, releasing the secbat!"))
