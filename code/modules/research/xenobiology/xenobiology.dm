...
/obj/item/slime_extract/purple/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			...
			to_chat(user, span_notice("I activate [src], and your body is refilled with fresh slime jelly!"))
			return 150
...
/obj/item/slime_extract/darkpurple/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			to_chat(user, span_warning("I activate [src], and a cloud of plasma bursts out of your skin!"))
			return 900
...
/obj/item/slime_extract/orange/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			to_chat(user, span_warning("I activate [src], and a cloud of smoke bursts out of your skin!"))
			return 450
...
/obj/item/slime_extract/blue/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			user.visible_message(span_danger("Foam spews out from [user]'s skin!"), span_warning("I activate [src], and foam bursts out of your skin!"))
			return 600
...
/obj/item/slime_extract/darkblue/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			to_chat(user, span_warning("I activate [src], and icy air bursts out of your skin!"))
			return 900
	...
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			if(user.gender != MALE && user.gender != FEMALE)
				to_chat(user, span_warning("I can't swap your gender!"))
				return
...
/obj/item/slime_extract/green/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, span_warning("I feel yourself reverting to human form..."))
		...
		if(SLIME_ACTIVATE_MAJOR)
			to_chat(user, span_warning("I feel yourself radically changing your slime type..."))
			if(do_after(user, 120, target = user))
...
/obj/item/slime_extract/black/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, span_userdanger("I feel something <i>wrong</i> inside you..."))
			user.ForceContractDisease(new /datum/disease/transformation/slime(), FALSE, TRUE)
			return 100

		if(SLIME_ACTIVATE_MAJOR)
			to_chat(user, span_warning("I feel your own light turning dark..."))
			if(do_after(user, 120, target = user))
...
/obj/item/slime_extract/oil/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		...
		if(SLIME_ACTIVATE_MAJOR)
			user.visible_message(span_warning("[user]'s skin starts pulsing and glowing ominously..."), span_userdanger("I feel unstable..."))
			if(do_after(user, 60, target = user))
				to_chat(user, span_userdanger("I explode!"))
	...
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			if(species.armor > 0)
				...
				return
			to_chat(user, span_notice("I feel your skin harden and become more resistant."))
			species.armor += 25
			addtimer(CALLBACK(src,PROC_REF(reset_armor), species), 1200)
			return 450

		if(SLIME_ACTIVATE_MAJOR)
			to_chat(user, span_warning("I feel your body rapidly crystallizing..."))
			if(do_after(user, 120, target = user))
				to_chat(user, span_warning("I feel solid."))
				user.set_species(pick(/datum/species/golem/adamantine))
				return
			to_chat(user, span_notice("I stop feeding [src], and your body returns to its slimelike state."))
...
/obj/item/slime_extract/bluespace/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			to_chat(user, span_warning("I feel your body vibrating..."))
			if(do_after(user, 25, target = user))
		...
		if(SLIME_ACTIVATE_MAJOR)
			if(!teleport_ready)
				to_chat(user, span_notice("I feel yourself anchoring to this spot..."))
				...
				if(teleport_x && teleport_y && teleport_z)
					...
					to_chat(user, span_notice("I snap back to your anchor point!"))
	...
	switch(activation_type)
		...
		if(SLIME_ACTIVATE_MAJOR)
			...
			if(istype(T))
				...
				to_chat(user, span_warning("I activate [src], and fresh air bursts out of your skin!"))
				return 600
...
/obj/item/slimepotion/afterattack(obj/item/reagent_containers/target, mob/user , proximity)
	. = ..()
	if (istype(target))
		to_chat(user, span_notice("I cannot transfer [src] to [target]! It appears the potion must be given directly to a slime to absorb.") )
		return
...
/obj/item/slimepotion/slime/docility/attack(mob/living/simple_animal/slime/M, mob/user)
	...
	if(M.rabid) //Stops being rabid, but doesn't become truly docile.
		to_chat(M, span_warning("I absorb the potion, and your rabid hunger finally settles to a normal desire to feed."))
	...
	M.docile = 1
	M.set_nutrition(700)
	to_chat(M, span_warning("I absorb the potion and feel your intense desire to feed melt away."))
...
/obj/item/slimepotion/slime/sentience/attack(mob/living/M, mob/user)
	...
	if(LAZYLEN(candidates))
		...
		to_chat(SM, span_userdanger("I am grateful to be self aware and owe [user.real_name] a great debt. Serve [user.real_name], and assist [user.p_them()] in completing [user.p_their()] goals at any cost."))
		if(SM.flags_1 & HOLOGRAM_1) //Check to see if it's a holodeck creature
			to_chat(SM, span_userdanger("I also become depressingly aware that you are not a real creature, but instead a holoform. Your existence is limited to the parameters of the holodeck."))
...
/obj/item/slimepotion/transference/afterattack(mob/living/M, mob/user)
	...
	if(SM.sentience_type != animal_type)
		to_chat(user, span_warning("I cannot transfer your consciousness to [SM].") )
		return ..()
	...
	to_chat(user, span_notice("I drink the potion then place your hands on [SM]..."))
